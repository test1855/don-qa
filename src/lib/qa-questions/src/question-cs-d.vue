<template>
  <div class="don-qa-question-cs-d" v-if="!fill">
  ...
  <div class="don-qa-question-cs-d" v-else>
     <pku-radio
        importKey="name"
        exportKey="id"
        :disabled="false"
  ...
  methods: {
    ...
    onQuestionEventHandler (val) {
      if (val) {
        this.question = val
        this.questionID++
        this.$refs.host.value = this.$refs.host.value + '  一题，访员提问是否符合题目原意？'
      }
    },
...
div.don-qa-question-cs-d >>> .don-qa-question-wrap label {
  font-size: 14px;
}
</style>
